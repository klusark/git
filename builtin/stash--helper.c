...
static int get_stash_info(struct stash_info *info, int argc, const char **argv)
{
	...
	} else if (strspn(info->commit, "0123456789") == strlen(info->commit)) {
		strbuf_addf(&commit_buf, "%s@{%s}", ref_stash, info->commit);
		revision = commit_buf.buf;
	}
...
static int do_drop_stash(const char *prefix, struct stash_info *info)
{
	...
	if (!ret) {
		if (!quiet)
			printf(_("Dropped %s (%s)\n"), info->revision, oid_to_hex(&info->w_commit));
	} else {
...
static int drop_stash(int argc, const char **argv, const char *prefix)
{
	...
	if (!info.is_stash_ref)
		return error(_("'%s' is not a stash reference"), info.commit);
