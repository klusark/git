...
static const char * const git_stash_usage[] = {
	...
	N_("                [-u|--include-untracked] [-a|--all] [-m|--message <message>]]"),
	N_("                [--] [<pathspec>...]]"),
	N_("git stash clear"),
	...
	const char **pathspecs)
{
	...
	if (!include_untracked && pathspecs && *pathspecs) {
		struct child_process cp = CHILD_PROCESS_INIT;
	...
	if (do_create_stash(&info, prefix, message, include_untracked, include_ignored, patch, pathspecs))
		return -1;
	...
	if (!patch) {
		if (include_untracked) {
			...
			if (pathspecs)
				argv_array_pushv(&args, pathspecs);
			cmd_clean(args.argc, args.argv, prefix);
		}

		if (pathspecs && *pathspecs) {
			struct argv_array args = ARGV_ARRAY_INIT;
			struct argv_array args2 = ARGV_ARRAY_INIT;
			struct child_process cp = CHILD_PROCESS_INIT;
			struct strbuf out = STRBUF_INIT;
...
static int push_stash(int argc, const char **argv, const char *prefix)
{
	...
	parse_options(argc, argv, prefix, options,
				git_stash_push_usage, PARSE_OPT_STOP_AT_NON_OPTION);
